...
def usage():
    ...
    print ("Usage: options are\n")
    print ("       -d: Domain to search or company name")
    print ("""       -b: data source: google, googleCSE,
                            googleplus, google-profiles, linkedin, twitter, 
                            yahoo, all\n""")
...
def start(argv):
    if len(sys.argv) < 4:
        ...
        sys.exit()
    try:
        opts, args = getopt.getopt(argv, "l:d:b:s:vf:")
    ...
    for opt, arg in opts:
        if opt == '-l':
            ...
            engine = arg
            if engine not in ("google", "googleCSE","virustotal", "googleplus", "google-profiles","linkedin", "twitter", "vhost", "yahoo","netcraft","all"):
                usage()
                print ("Invalid search engine, try with: google, googleCSE, virustotal, netcraft, googleplus, google-profiles, linkedin, twitter, vhost, yahoo, all")
                sys.exit()
            else:
                pass
    if engine == "google":
    ...
    if filename != "":
        ...
        try:
            ...
            file.write('<?xml version="1.0" encoding="UTF-8"?><the Info>')
            for x in all_emails:
